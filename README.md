...
started, and any failure is logged without taking the shell down. When an
update feed is configured (see below), the menu also shows the current update
state as a disabled status line plus a clickable **Check for updates** item
that re-runs the check on demand. The
...
**Update state in the tray**: with a feed configured, the tray menu shows the
result of the last check as a disabled status line — `Update available —
restart to install` when a newer release exists, `Up to date`, or the failure
reason (`feed unreachable` / `unrecognized feed`) — plus a clickable
**Check for updates** item that re-runs the check on demand and refreshes the
menu with the new status. Without `OCR_UPDATE_FEED` none of these items exist
and the tray stays exactly as before.
