...
await runExplorer({ workspace: ws } as PilotConfig, scratch, {
  save: (st) => {
    ...
    P3_052_ORDER.push("claim");
  },
  // P3-052 hardening: the scratch already carries freshly copied dists (see
  // setup) — inject the recorder so the runner's rebuild step is proven, not
  // skipped, and no scratch build slows the proof run.
  rebuild: (rebuildWs) => {
    P3_052_ORDER.push("rebuild");
    return { ok: rebuildWs === ws, output: "" };
  },
});

proof("claim persisted before the run", saved.length === 1 && saved[0] === today, `explorerLast=${saved[0] ?? "none"}`);
// P3-052 hardening: the pre-journey rebuild is wired into the real flow —
// exactly one call, after the once-per-day claim (so a failed build can never
// retry-build every scheduler cycle) and before the agent spawns.
proof(
  "P3-052: bundle rebuild wired before the journey (once, after the claim)",
  P3_052_ORDER.length === 2 && P3_052_ORDER[0] === "claim" && P3_052_ORDER[1] === "rebuild",
  P3_052_ORDER.join(" -> ") || "no calls",
);
