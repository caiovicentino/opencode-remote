      ...
      "P1-089: boot-2 paired hook rendered",
            ...
            if (back.ok) {
              ...
              const rowProbe = await waitProbe(
                ...
                // after chat-back — reclaim the board on each re-probe. The
                // Go-menu pane action resets to the home when no chat is
                // active, so the drawer path is the honest reclaim.
                () => {
                  run("P1-089: reclaim — open the drawer", ["click", ".shell-menu"], 15_000, localEnv2);
                  run("P1-089: reclaim — chats destination", ["click", '.drawer-row[data-dest="chats"]'], 15_000, localEnv2);
                },
              );
              if (!rowProbe) {
