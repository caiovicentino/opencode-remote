...
| raw upstream error on the first message (P2-210) | no provider on the machine hosting the daemon has a credential (or the credentials expose no models). The composer now warns BEFORE the send: a calm line above it derived from the same provider catalog the context gauge already fetches. The indicator describes the machine hosting the daemon — never the phone — and **never blocks sending**. `GET /__ocr/model/status` returns `{ available, state, message }`; the verdict is re-observed lazily by that route when it is not ready (P3-397, at most once per `OCR_MODEL_READINESS_MIN_MS`, default one minute — `OCR_MODEL_READINESS_DISABLE=off` turns it off), so configuring a credential mid-session is picked up without a restart; `OCR_MODEL_BLOCK=1` on the daemon forces the no-provider verdict for deterministic screenshots |
