...
| tray keeps promising an update but nothing is downloading (P2-330) | the background download failed outright (network drop, full disk, rejected package). The status line flips to `Update check failed — download failed` on the spot — no path, no URL, no raw error — and the next check runs on the failure backoff (15 min, doubling per consecutive failure, 5-min floor) instead of waiting out the six-hour interval; one `update status (…): update-download-failed` line lands in `desktop.log` |
