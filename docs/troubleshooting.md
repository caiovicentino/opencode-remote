...
| `relay connection lost` in a loop and every paired device drops at the same time (P3-344) | a client with a revoked/unknown identity kept reconnecting (old PWA tab, auto-reconnect every ~8s) and the daemon used to close its **single shared relay socket** to shoo it away — every other device died with it, then all reconnected together. Fixed: the daemon now drops the rejected pub silently (warn + audit throttled to one line per device fingerprint per 60s) and revoking a device never closes the relay socket either — the other devices' sessions are untouched. If you still see the loop, update/restart the daemon |
| `relay connection lost; retrying` | relay down or tailscale dropped — check the relay terminal/service |
| the daemon keeps re-dialing the relay every few seconds and no log line says why (P2-260) | a dial failure is now classified by the daemon itself: unresolved name, refused connection, connect timeout, expired / untrusted / hostname-mismatched relay certificate. The `relay dial failed` log line carries the kind plus one static pt-BR hint instead of the raw Node message (which embeds the relay host and port), the same verdict rides `GET /api/health` as `relayRetry.lastDial = { kind, hint }` next to `lastClose`, and a permanent cause waits 60s (bad address) or 5min (bad certificate) before re-dialing so the machine stops hammering an address that will not fix itself — fix the address or the relay certificate and the daemon reconnects on its own |
| the daemon reconnects forever to a hosted relay that was just updated (P2-335) | an incompatible relay wire version used to look exactly like a temporarily-down relay. The daemon now reads the relay's own `/healthz` `protocol` field (P2-331) once its reconnect loop has failed 3 dial cycles in a row (throttled to one probe per 10 minutes, 5s timeout, 4KB body cap): `GET /api/health` gains the additive `relayProtocol` object — `mismatch` names the real problem ("o relay fala um protocolo de fio diferente deste app — atualize o app ou o relay hospedado") with one warn log line per state change, `legacy` (relay predates the field) and every failure/timeout stay `unknown` and change nothing. Since P2-338 the desktop UI reads the same field: the pairing QR's relay-link line and the tray stop saying "reconnecting, wait and rescan" and name the mismatch instead ("o relay hospedado fala um protocolo de fio diferente deste app — atualize o app ou o relay hospedado e aguarde a reconexão") — a live link still wins over a stale mismatch, and the tray treats it as a warning like a refusal, with no new menu item. Fix: update the app build or roll the hosted relay back to a wire version this build speaks — the reconnect loop itself is unchanged. Since P2-345 the diagnostic bundle carries the same verdict in its `relay link:` line (the closed-set state only, never the message, an address or a relay id) |
| the daemon reconnects forever to a hosted relay that was just updated (P2-335) | an incompatible relay wire version used to look exactly like a temporarily-down relay. The daemon now reads the relay's own `/healthz` `protocol` field (P2-331) once its reconnect loop has failed 3 dial cycles in a row (throttled to one probe per 10 minutes, 5s timeout, 4KB body cap): `GET /api/health` gains the additive `relayProtocol` object — `mismatch` names the real problem ("o relay fala um protocolo de fio diferente deste app — atualize o app ou o relay hospedado") with one warn log line per state change, `legacy` (relay predates the field) and every failure/timeout stay `unknown` and change nothing. Since P2-339 a recorded `mismatch` also paces the daemon itself: the reconnect wait takes a documented 5-minute floor (surfaced as `relayRetry.floorSource: "protocol-mismatch"`), so an app that knows it cannot talk to the relay stops hammering it at transient-drop pace — the wake/redial route can still anticipate that wait after you update the app or the relay (one redial per 10s). Fix: update the app build or roll the hosted relay back to a wire version this build speaks, then the daemon self-heals as soon as the relay delivers a frame |
...
| `voice replies unavailable` (P2-298) | spoken replies are an **optional** host capability (edge-tts CLI). The phone shows a short actionable pt-BR sentence instead of a raw English install instruction, and the daemon serves the same verdict on `GET /__ocr/voice/tts-status` (`{ available, voice, voices, langs, state, message }`) — detection now uses the platform's own locator, so a Windows host with the tool installed is recognized, and an install made after boot is picked up by the lazy re-probe. `OCR_TTS_BLOCK=1` on the daemon forces the missing-tool verdict for deterministic screenshots |
| raw upstream error on the first message (P2-210) | no provider on the machine hosting the daemon has a credential (or the credentials expose no models). The composer now warns BEFORE the send: a calm line above it derived from the same provider catalog the context gauge already fetches. The indicator describes the machine hosting the daemon — never the phone — and **never blocks sending**. `GET /__ocr/model/status` returns `{ available, state, message }`; `OCR_MODEL_BLOCK=1` on the daemon forces the no-provider verdict for deterministic screenshots. Since P3-397 the verdict is re-checked lazily on that route while it is not ready (at most once per `OCR_MODEL_READINESS_MIN_MS`), so adding the credential is picked up without restarting the daemon. On the desktop shell the `no-provider`/`no-model` hint resolves to dedicated copy (P3-396 — the reader IS the machine manager) with three real actions: **Copy login command** puts the official `opencode auth login` on the clipboard, **Open setup instructions** opens the official provider docs through the shell's external-link gate, and **Check again** re-probes the verdict — the whole hint disappears by itself once the credential is in (always a terminal state, never a permanent spinner). The phone keeps the daemon's own sentence with no actions |
...
| the desktop app says the agent server was never installed (P3-392) | the machine hosting the daemon has **no opencode binary at all** (connection refused + `binaryFound: false`, the P2-149 split). The desktop shell resolves the notice to "The agent server is not installed on this machine" and carries the whole install journey in the calm card and the welcome wizard: **Copy install command** puts the official per-platform one-liner on the clipboard (macOS/Linux `curl -fsSL https://opencode.ai/install \| bash`; Windows `irm https://opencode.ai/install.ps1 \| iex`), **Open install instructions** opens the official docs through the shell's external-link gate, and **Check again** re-runs the health poll — the notice goes away by itself once the install finishes (always a terminal state, never a permanent spinner). The phone keeps the previous generic copy. `OCR_OPENCODE_MISSING=1` on the daemon forces the binary-absent half of the split for deterministic screenshots (the upstream probe itself stays real) |
| raw write/filesystem errors mid-conversation, or a full disk (P2-215) | the volume hosting the daemon's state directory (`~/.opencode-remote/`) is out of free space — the daemon writes session artifacts, upload staging, the audit log and the state file there. Settings → machine section shows one calm line asking the machine's owner to free space; the indicator describes the machine hosting the daemon — never the phone — and **never blocks anything** except one documented exception: since P3-462 the chat composer disables its **+ attach** button while the verdict is critical, with the machine's own phrase under the composer (the app's static line when the machine sent none); every other control stays enabled, low only adds a discreet warning, and ok/unknown verdicts stay silent. Thresholds (most severe wins): warning below 2 GB free or 10% of the volume, critical below 500 MB free or 5%. Since P2-347 the daemon re-reads the volume lazily at the upload surface too (at most once per `OCR_READINESS_MIN_MS`, honoring `OCR_READINESS_DISABLE`, no new timer) and with **critical** refuses a new upload up front with HTTP 507 and the same phrase before any byte is written — the refusal body and `GET /api/health` both carry the machine's own sentence; the phone's own upload-error path keeps its generic message for now (a follow-up surfaces the phrase there), and the sentence already shows under the composer through the settings read (P3-462); `ok`, `low` and unknown keep every route as before, and the payload also carries the additive `diskCheckedAt` instant of the last reading. To fix it, delete or archive large files on that machine (old session artifacts are trimmed automatically by the retention janitor) and keep at least a few GB free; `OCR_DISK_FULL=1` on the daemon forces the critical verdict for deterministic screenshots and `OCR_DISK_OK=1` is the symmetric hatch forcing the ok verdict (hermetic tests on a genuinely full host) |
...
| the first-boot card says the app cannot write its own data (P2-346) | the shell runs ONE storage probe at boot (writes and deletes a small temp file in the app's data folder, before the local daemon starts) and the calm card replaces "Retrying automatically…" with the verdict's phrase when the folder refuses the write: "o app não tem permissão para gravar na pasta de dados dele" → fix the folder's permissions (the volume/user is blocking the app); "a pasta de dados do app está somente leitura" → remount the volume with write access; "o disco deste computador está cheio" → free disk space. The verdict is logged in `desktop.log` as `storage probe: <state> (<phrase>)` and the card keeps the patient retry line whenever the folder is fine or the verdict is missing. The phrase never carries a path, a user name or the raw system error |
| I escaped the wizard's pairing error into the manual screen and there is no QR scan there (P3-427) | while the local agent is down, the manual pairing ceremony drops the QR-scan entry, the phone-pairing entry and the intro that promised "the daemon's QR" — all of them need a QR the down agent cannot mint — and shows a calm verdict card ("The local agent is not running.") with its own **Reconnect now**. The paste box stays: a pairing code from another machine already running the app still works. Once the daemon answers again, the full ceremony (intro, host entry, scan option) comes back by itself |
| the machine only reaches the internet through a proxy (P2-285) | the desktop shell reads the machine's proxy environment once at boot and follows the **system proxy** by default; a valid fixed address from the proxy variables is applied to the session with loopback always bypassing it, so the local daemon bridge never routes through the proxy. One `proxy:` line in `desktop.log` (and one in the diagnostic bundle) carries the mode, its origin (your choice or the machine environment) and the reason — never the address or credentials. You can also pick the proxy by hand in Settings → **Machine proxy** (system / no proxy / fixed address): the choice is stored on this machine and takes effect right away — the running session is reconfigured on the spot, and a changed fixed address also restarts the daemon sidecar so the relay link follows (P2-307); saving the same choice again changes nothing. Since P2-350 the shell also watches the Electron `login` event: when the proxy itself demands authentication (HTTP 407), one `proxy auth:` line lands in `desktop.log` per state change, the diagnostic bundle carries the same verdict in its `proxy auth:` line, and the tray's update error names the proxy instead of a generic feed failure. The shell never asks for or stores proxy credentials — the challenge is cancelled, so configure the network's authenticated proxy at the OS level (or use the direct mode) rather than expecting a prompt |
| report a problem with everything attached | desktop → Settings → **Diagnostics → Copy diagnostic**: versions, platform, daemon state, last 40 `desktop.log` + 20 `daemon-sidecar.log` lines and the crash-file names, on your clipboard. Since P2-345 it also carries the verdicts support needs first: `update rollout:` (why this machine did not receive the latest version), `relay link:` (why the phone cannot connect — the daemon↔relay link state) and `sidecar wedge:` (whether the local daemon stopped answering while alive). Since P2-350 it also carries `proxy auth:` (whether a network challenge came from the proxy). Each carries the state/decision and a short stable reason only — never an installation id, a bucket, a URL, a host, a port or a relay instance id. No secrets (apiToken/allowlist/pairing URI never included) |
...
| tray says the update was postponed (P2-264) | the scheduled recheck measures the free space of the volume hosting the app's data directory before starting a download, and the disk did not have room for the package plus the unpacked copy (or the space could not be measured). Free up disk space and the next scheduled tick retries by itself, or tray → **Check for updates** to retry right now; one `update space:` line in `desktop.log` records each decision, and nothing is ever cancelled, deleted or installed by the gate |
| two machines run different versions right after a release (P2-342) | that is the **gradual rollout** working, not a broken feed: a release may ship with a rollout percentage (`rolloutPercent` in the macOS JSON feed, `stagingPercentage` in Windows `latest.yml`), and each install sits in one of 100 buckets derived from its own private installation id (a random UUID written once to the app's data directory — never from hardware, keys or pairing). A machine receives the version only while its bucket is below the published percentage, so during the first hours after publishing some machines update while others stay put; the percentage grows and everyone converges. Tray → **Check for updates** bypasses any percentage between 1 and 99 on that machine right away (a published **0** holds everyone back, even that click). One `update rollout:` line in `desktop.log` records each verdict transition, and the diagnostic bundle carries the same verdict in its `update rollout:` line (decision + stable reason only — never the installation id, the bucket or the percentage); an absent, unreadable or out-of-range percentage offers to every machine — nothing is ever held by doubt |
| the installed version can't open a window at all (P2-270) | the shell remembers which version last truly opened and how many consecutive openings died before a useful window (`boothealth.json` in the app's data directory): past a documented floor the tray shows `versão sem janela útil: atualização automática suspensa`, the automatic update check is skipped for that execution (boot + scheduled rechecks only — **Check for updates** still works), one `boot health:` line lands in `desktop.log` and a one-question dialog offers **Copiar diagnóstico** (the P2-163 bundle) or **Seguir assim mesmo**. To get out for good, install the previous release from the GitHub releases page (or fix the machine) — the app itself never rolls back, uninstalls, deletes or installs anything, and the record heals itself the first time the window truly opens again. Since P2-291, while that verdict stands the update guard also refuses to re-download or re-offer the very version the feed keeps advertising when it is the one running (`update guard: recusar-oferta` in `desktop.log` and in the diagnostic bundle) — tray → **OpenCode Remote — retomar atualização automática** records your release and re-checks immediately, and a genuinely new version is followed like always |
...
| mac download says "damaged and can't be opened" | that is an unsigned/stale DMG, not this project's release path: since P2-170 the desktop-dmg job runs the Gatekeeper verdicts (`codesign` verify, `spctl` assess, `stapler` validate — `scripts/gatekeeper-verify.ts`) on the packaged app before uploading, and since P2-295 it runs the same three on every DMG **container** itself (`spctl -t open` + `stapler validate` on each image you double-click), failing the release when a container's own verdicts drift from the documented packaging shape (unsigned container, rejection and no staple are the baseline; a broken verification, unreadable tool output or a signed-but-blocked container abort the job) |
...
| Windows: "Check for updates" says an update is ready — now what? (P2-233) | that explicit click downloaded the installer named by `latest.yml` into the `update-staging` folder inside the app's user data directory, verified its sha512 against the digest the feed publishes and opened Explorer with the file already selected — **double-click it yourself**; the app never runs the installer. If the digest did not match, the file was deleted on purpose (one `win update:` line in `desktop.log` explains it) and the app falls back to the release page — click **Check for updates** again to retry the verified download. Since P2-301 the download itself is skipped (one `update install not offered` line in `desktop.log`) when the running executable sits in a user-temp extraction of a zip or on a network share — the installer would write an installed copy the person never reopens — so install the app once with the setup exe and reopen it from the installed copy to receive updates again |
...
- Site browsing (P2-287) follows the verdict table written in
  ...
  reports the verdict on the channel the screen already reads; a daemon that
  does not report it simply renders no row.
- Voice transcription (P2-297) follows its own closed table in the same
...
- Deterministic screenshots: `OCR_OPENCODE_OLD=1` forces the too-old agent
  ...
  indicators above. In the app itself, localStorage hatches force the
  ...
  `ready`/`complete`, so it can never fabricate a green row; anything else is
  ignored), the real payload always wins when it exists, and no phrase is
...
per 10 s. Since P2-339 the same route also anticipates a recorded
`protocol-mismatch` floor (5 minutes, P2-335) — the explicit redial after you
...
- An automatic login boot stays **quiet** (P2-348): the main window is ready
  ...
  invite brings the window up exactly as it was left (maximized included).
  The decision rides the boot log (`[desktop] login launch: …`).
