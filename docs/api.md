...
Since P2-287 (scope amended by review) the existing `GET /__ocr/settings`
read mirrors `browseState`/`browseMessage` additively — appended after the
`disk` anchor, never renamed or repositioned, re-probed under the same lazy
`OCR_READINESS_*` policy — so the Settings machine-state panel renders the
