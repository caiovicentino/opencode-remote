...
export function buildMissionPrompt(): string {
  return [
    ...
    `- Com repoUrl, avise o usuário em uma frase como a frota trabalha no repo dele: clona o repo nesta máquina, abre PRs (branches pilot/<id> e pilot/meta, squash merge em main, incluindo um BACKLOG.md no formato do pilot se o repo não tiver) usando as credenciais do gh desta máquina — nada é commitado direto em main.`,
    `- Se o pilot substituiu um modelo pedido (id indisponível no opencode), o GET /api/pilot-mission do daemon devolve modelSubstitutions (papel, modelo pedido, modelo usado); quando o usuário perguntar qual modelo está rodando, consulte e diga a substituição em vez de repetir o pedido.`,
